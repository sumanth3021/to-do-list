...
const getLocal = () => {
  ...
  if (tasksLocal) {
    ...
    console.warn("No tasks found in local storage.");
  }
};
...
const renderTasks = () => {
  tasksContainer.innerHTML = ""; // Clear the tasksContainer
  const categoryTasks = tasks.filter(
    (task) =>
      task.category.toLowerCase() === selectedCategory.title.toLowerCase()
  );
...
// Check if DOM elements are present before proceeding
if (!categoriesContainer || !screenWrapper || !menuBtn || !backBtn || !tasksContainer || !numTasks || !categoryTitle || !categoryImg || !categorySelect || !addTaskWrapper || !addTaskBtn || !taskInput || !blackBackdrop || !addBtn || !cancelBtn || !totalTasks) {
...
backBtn.addEventListener("click", () => {
  ...
  categoryImg.src = `images/${selectedCategory.img}`;
  renderTasks();
});
